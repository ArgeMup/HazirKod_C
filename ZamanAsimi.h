...
{	
	...
	_Ortak_Tip_uint32_t_ (*Islem)(struct s_Gorev_ * Gorev);
...
{
	void (*SayaciGuncelle)(struct s_An_ * Simdi, _Ortak_Tip_uint32_t_ Hatirlatici);
	_Ortak_Tip_uint32_t_ Hatirlatici;
	...
	struct
	{
		...
		struct s_Gorev_ Gorevler[ZamanAsimi_AcKapat_Gorev_Adet];
		#endif
	}_IcKullanim_;
};  
	...
	void ZamanAsimi_Baslat_An(struct s_An_ * An);

	_Ortak_Tip_Isaretci_ ZamanAsimi_Gorev_Ekle(struct s_Gorev_ Gorev);
