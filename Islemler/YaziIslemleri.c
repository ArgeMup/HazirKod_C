...
#include "YaziIslemleri.h"
	...
	Tip_i32 YI_BulAyiklaKopyala(Tip_char * Kaynak, Tip_char * ArananBaslangic, Tip_char * ArananBitis, Tip_char * Hedef, Tip_u32 HedefKapasite)
	{
		...
		if (Kaynak == Tip_null || Hedef == Tip_null || HedefKapasite == 0) return -1;
		...
		if ( (Adet + 1 /*\0*/) > HedefKapasite) return -2;
	...
	{
		if (Kaynak == Tip_null || Hedef == Tip_null || HedefKapasite == 0) return Tip_null;
