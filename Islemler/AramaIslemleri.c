...
#include "AramaIslemleri.h"
	...
	Tip_i32 AI_Bul_Bayt(Tip_Isaretci Kaynak, Tip_u32 KaynaktakiBilgiMiktari, Tip_u8 ArananBilgi)
	{
		...
		{
			if (Isaretci_Icerigi(Kaynak, i, Tip_u8, Tip_u8) == ArananBilgi) return i;
		}
	...
	Tip_i32 AI_Bul_Blok(Tip_Isaretci Kaynak, Tip_u32 KaynaktakiBilgiMiktari, Tip_Isaretci ArananBilgi, Tip_u32 ArananBilgiMiktari)
	{
		...
		TekrarAra:
		Konum_IlkKarakter = AI_Bul_Bayt(Isaretci_Konumlandir(Kaynak, Konum_Yedek, Tip_u8, Tip_void), KaynaktakiBilgiMiktari - Konum_Yedek, Isaretci_Icerigi(ArananBilgi, 0, Tip_u8, Tip_u8));
		...
		{
			if (Isaretci_Icerigi(Kaynak, Konum_IlkKarakter, Tip_u8, Tip_u8) !=
				Isaretci_Icerigi(ArananBilgi, Sayac_Konum_Aranan, Tip_u8, Tip_u8)) goto TekrarAra;
	...
	Tip_i32 AI_Bul_Blok_Sondan(Tip_Isaretci Kaynak, Tip_u32 KaynaktakiBilgiMiktari, Tip_Isaretci ArananBilgi, Tip_u32 ArananBilgiMiktari)
	{
		...
		TekrarAra:
		Konum_IlkKarakter = AI_Bul_Bayt_Sondan(Kaynak, Konum_Yedek, Isaretci_Icerigi(ArananBilgi, 0, Tip_u8, Tip_u8));
		if (Konum_IlkKarakter < 0) return -2;
		Konum_Yedek = Konum_IlkKarakter;
						...
						Tip_bool SadeceIlkBuldugunuDegistir)
	{
		...
		{
			Tip_i32 Konum_Aranan = AI_Bul_Blok(Isaretci_Konumlandir(Kaynak, Konum_Kaynak, Tip_u8, Tip_void), KaynaktakiBilgiMiktari - Konum_Kaynak, ArananBilgi, ArananBilgiMiktari);
			...
			_Islem_memcpy_(Isaretci_Konumlandir(Hedef, Sayac_HedefKullanildi, Tip_u8, Tip_void), Isaretci_Konumlandir(Kaynak, Konum_Kaynak, Tip_u8, Tip_void), Konum_Aranan);
			...
			_Islem_memcpy_(Isaretci_Konumlandir(Hedef, Sayac_HedefKullanildi, Tip_u8, Tip_void), YeniBilgi, YeniBilgiMiktari);
		...
		if (Konum_Kaynak < KaynaktakiBilgiMiktari)
		{
			...
			_Islem_memcpy_(Isaretci_Konumlandir(Hedef, Sayac_HedefKullanildi, Tip_u8, Tip_void), Isaretci_Konumlandir(Kaynak, Konum_Kaynak, Tip_u8, Tip_void), Sayac_Bulundu);
